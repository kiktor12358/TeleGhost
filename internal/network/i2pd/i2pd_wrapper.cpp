 ...
 *
 * Build with:
 *   g++ -std=c++17 -c i2pd_wrapper.cpp -I../i2pd/libi2pd -I../i2pd/i18n -I../i2pd
 */
...
// Check if router is running and ready
int i2pd_is_running() {
    if (!g_running) return 0;
    
    // Check if router context is ready
    try {
        if (i2p::context.IsReady()) {
            return 1;
        }
    } catch (...) {
        return 0;
