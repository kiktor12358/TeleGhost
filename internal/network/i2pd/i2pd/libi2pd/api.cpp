...
void InitI2P(int argc, char *argv[], const char *appName) {
  ...
  i2p::config::ParseCmdline(argc, argv,
                            true); // ignore unknown options and help
  i2p::config::Finalize();
  ...
  bool checkReserved;
  i2p::config::GetOption("reservedrange", checkReserved);
  i2p::transport::transports.SetCheckReserved(checkReserved);

  i2p::context.Init();
}

void TerminateI2P() { i2p::crypto::TerminateCrypto(); }
...
void StartI2P(std::shared_ptr<std::ostream> logStream) {
  if (logStream)
    ...
    std::string log;
    i2p::config::GetOption("log", log);
    if (log == "file") {
      std::string logfile;
      i2p::config::GetOption("logfile", logfile);
      if (logfile == "")
        logfile = i2p::fs::DataDirPath(i2p::fs::GetAppName() + ".log");
...
void RunPeerTest() { i2p::transport::transports.PeerTest(); }
...
CreateLocalDestination(const i2p::data::PrivateKeys &keys, bool isPublic,
  ...
  auto localDestination =
      std::make_shared<i2p::client::RunnableClientDestination>(keys, isPublic,
                                                               params);
...
void DestroyLocalDestination(
    std::shared_ptr<i2p::client::ClientDestination> dest) {
  if (dest)
    dest->Stop();
}

void RequestLeaseSet(std::shared_ptr<i2p::client::ClientDestination> dest,
                     const i2p::data::IdentHash &remote) {
  if (dest)
    dest->RequestDestination(remote);
}

std::shared_ptr<i2p::stream::Stream>
CreateStream(std::shared_ptr<i2p::client::ClientDestination> dest,
             const i2p::data::IdentHash &remote) {
  if (!dest)
    return nullptr;
